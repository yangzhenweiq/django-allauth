...
class LonganTests(OAuth2TestsMixin, TestCase):
    ...
    def test_longan_compelete_login_401(self):
        from allauth.socialaccount.providers.longan.views import \
            LonganOAuth2Adapter
        ...
        adapter = LonganOAuth2Adapter(request)
        ...
        with patch(
                'allauth.socialaccount.providers.longan.views'
    ...
    def test_account_connect(self):
        ...
        self.assertTrue(SocialAccount.objects.filter(
            user=user,
            provider=LonganProvider.id).exists())
...
class AppInSettingsTests(LonganTests):
    """
